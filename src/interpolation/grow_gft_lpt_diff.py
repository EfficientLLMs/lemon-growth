...
class args:
    ...
    eval_file = "eval/eval_lora_svd.json"
...
def playground():
    ...
    # Initialize an empty lora adapter of a desired rank for the large pretrained model
    ...
    large_pretrained_model = get_peft_model(large_pretrained_model, lora_config)

    # Add the difference (in low rank) to the lora adapter of the large pretrained model
    add_diff_as_lora(
    ...
    # Check to see if base model was modified
    for name, param in large_pretrained_model.named_parameters():
        if 'lora' in name:
            print('-' * 50)
            print(name)
            print(param)
...
def main():
    ...
    # Load the large_lora_model from disk
    large_lora_model = eval_utils.load_model("tmp_large_pretrained_model")
    ...
    # Prepare the models for evaluation
    large_lora_model = accelerator.prepare(
        large_lora_model,
    )
    ...
    # Evaluate the large_lora_model with the grown difference
    avg_eval_loss, metrics = eval_utils.evaluate_model(
        large_lora_model, eval_dataloader, accelerator, tokenizer
    )

    eval_results["large_lora_model_diff_svd"] = {
        "model_name": args.large_pretrained_model_cpt,
        "avg_eval_loss": avg_eval_loss,
        **metrics
    }
    ...
    print(eval_results["large_lora_model_diff_svd"])

    del large_lora_model
    clean_memory()
